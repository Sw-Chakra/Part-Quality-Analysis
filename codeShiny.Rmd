---
title: Data Visualization using Shiny (Interactive)
runtime: shiny
output: 
  html_document:
    df_print: paged
...
This is an interactive page (user selects the inputs to see a particular plot), and can be hosted only through Shiny Server (public) or through personal laptop which has the required packages and applications to connect with Shiny. This page might appear blank due to inadequacy of resources, hence would be demonstrated during the onsite presentation. Kindly refer to the "STATIC VISUALIZATION TAB" for initial reference.
...
shinyUI <-pageWithSidebar(
  ...
  # Application title
  headerPanel("Enter Distribution Inputs"),
  
  sidebarPanel(
                selectInput("dataset", 
                            "Choose a dataset:", 
                            choices = c("D71", "D72", "D73","F11","F12","F13","M13","M15")
                            ),
                selectInput("feature", 
                            "Choose a feature:", 
                            choices = c("Diameter", "Distance", "Cylindricity")
                )
              ),
...
shinyServer <-(function(input, output) {
  ...
  output$plot1 <- renderPlot({
    dist_summary(dataset = get(input$dataset),text = input$dataset, feature=input$feature)
  })
...
shinyUI <-pageWithSidebar(
  ...
  # Application title
  headerPanel("Distribution Comparison"),
  
  sidebarPanel(
                selectInput("fixture", 
                            "Choose a fixture:", 
                            choices = list("FIXTURE_D71", 
                                        "FIXTURE_D72", 
                                        "FIXTURE_D73",
                                        "FIXTURE_F11",
                                        "FIXTURE_F12",
                            ...
                            selected = list("FIXTURE_D72","FIXTURE_F12"),
                            multiple = TRUE
                            ),
                selectInput("feature", 
                            "Choose a feature:", 
                            choices = colnames(measurement_all[,2:50]),
                            selected = "C0009DE083_SC08_DIA"
                ),
  ...
  mainPanel(
                h6(textOutput("multiple_fixture")),
...
shinyServer <-(function(input, output) {
    ...
    output$multiple_fixture <- renderText({
    x <- input$fixture
    paste("Comparison of Distribution of ", input$feature, " across ",
          paste(as.character(x), collapse = ", "))
  })
  
  output$plot_dist <- renderPlot({
    distribution_comparison(fixture_list= as.character(input$fixture),
                        feature=input$feature,
                        input_range = as.numeric(c(input[["range"]][1],
                                                   input[["range"]][2]))
                        )
  })
...
shinyUI <-pageWithSidebar(
  ...
  # Application title
  headerPanel("Compare Features Across Features"),
  
  sidebarPanel(
                    # data select
                selectInput("feature_category", 
                            "Feature Category", 
                            as.list(feature_cate), 
                            selected="Diameter"),
  ...
  mainPanel(
                  plotOutput("plot3"),
                  h3(textOutput("featureCate_select")),
...
shinyServer <-(function(input, output) {
  ...
  output$featureCate_select <- renderText({
    x <- input$feature_category
    paste("The Feature Category is",x)
  })
  
  # varible select
  output$choose_columns <- renderUI({
    req(input$feature_category)
    
    # Get the variable category with the appropriate name
    if(input$feature_category == "Diameter") {
      colnamess <- as.vector(unique(dia_features))
    } else if (input$feature_category == "Distance"){
      colnamess <- as.vector(unique(dist_features)) 
    }else if (input$feature_category == "Flatness"){
      colnamess <- as.vector(unique(flt_features)) 
    }else if (input$feature_category == "Cylindricity"){
      colnamess <- as.vector(unique(cyl_features)) 
    }else if (input$feature_category == "TP"){
      colnamess <- as.vector(unique(tp_features)) 
    }else {
      colnamess <- as.vector(unique(dia_features))
    }
  ...
  output$variables_select_value <- renderText({
    paste("The variables selected is",input$columns)
  })
...
#compare_means("Cylindricity", "C0009P0053_SC05_CYL")
   output$plot3 <- renderPlot({
     req(input$feature_category)
     req(input$columns)
    compare_means(input$feature_category, input$columns)
  }) 
...
shinyUI <-pageWithSidebar(
  ...
  # Application title
  headerPanel("Compare Features Across Fixtures"),
  
  sidebarPanel(
                selectInput("feature", 
                            "Choose a feature:", 
                            choices = c("Diameter", "Distance","Flatness", "Cylindricity", "TP")
                )
              ),
...
shinyUI <-pageWithSidebar(
  ...
  # Application title
  headerPanel("Compare Features Across Machines"),
...
shinyServer <-(function(input, output) {
  ...
  output$plotM <- renderPlot({
    compare_prop_machines(input$feature)
  })
...
shinyUI <-pageWithSidebar(
  ...
  # Application title
  headerPanel("Compare Features for aggregate analysis"),
...
shinyServer <-(function(input, output) {
  ...
  output$plot_aggre <- renderPlot({
    compare_prop_aggregate(input$feature)
  })
...
aggregate_score <-ggplot(data=feature_issue_score, aes(x=reorder(feature,-score), y=score)) +
...
shinyUI <-fluidPage(
  fluidRow(
    column(width = 12,class = "well",
      h4("Brush and double-click to zoom the feature scores"),
...
shinyServer <-(function(input, output) {
  ...
  output$plot1 <- renderPlot({
    aggregate_score + coord_cartesian(xlim = ranges$x, ylim = ranges$y, expand = FALSE)
  })
...
Top 5 extent/ degree/ score of failure**:
...
With this we complete visual analysis of differences of features across fixture types.  


#### Features with largest difference between fixtures:

In order to quantify the differences, we would assess the *p-value* of each feature and compare it among fixture types. Hence we perform the **test static** to get the p-value.  
...
shinyUI <-pageWithSidebar(
  ...
  # Application title
  headerPanel("How different are the features across features"),
  ...
  mainPanel(
                verbatimTextOutput("Compare difference across features"),
                plotOutput("plot6")
          )
),
...
# check <-diff_fixture_table("S0009DA001_PLN_A_FLT")
# nrow(check)
```
...
shinyUI <-pageWithSidebar(
  ...
  # Application title
  headerPanel("How different are the features across fixtures"),
...
shinyServer <-(function(input, output) {
  ...
  output$table <- renderTable({
    diff_fixture_table(input$feature)
  })
...
#### Features with largest difference between machines:
...
shinyUI <-pageWithSidebar(
  ...
  mainPanel(
                verbatimTextOutput("Compare difference across machines"),
                plotOutput("plot7")
          )
),
...
p value comparison for 49 features:
```{r include=FALSE}
...
# select the features which are significantly different p-value < 0.05:

# Diameter features
Machine_dia <-Machine_diff[,c(1,2,4,6)] %>% 
              filter(feature_name %in% dia_features) %>% 
              filter(p_value <= 0.05) %>% 
              gather("Machine","Mean",2:3) %>%
              ggplot(aes(x=feature_name, y=Mean, fill=Machine,group=Machine))+ 
              geom_bar(stat="identity", color="black", 
                      position=position_dodge(), 
                      width=0.3) + 
              theme_minimal() + geom_hline(yintercept = 0)+
              theme(axis.text = element_text(angle = 45,hjust=1,size = 8)) +
              scale_fill_manual(values=c('red3','salmon'))+
  labs(title="Compare Means between Machines for Diameters difference with significant p-value", 
       x="Diameter Features", y = "Mean value of Measurement") 

# Distance features
Machine_dist <- Machine_diff[,c(1,2,4,6)] %>% 
                ...
                theme(axis.text = element_text(angle = 45,hjust=1,size = 8)) + 
                scale_fill_manual(values=c('red3','salmon'))+
                labs(title="Compare Means between Machines for Distance difference with significant p-value", 
                     x="Distance Features", y = "Mean value of Measurement")
  
  
# Flatness feautures
Machine_flt <-Machine_diff[,c(1,2,4,6)] %>% 
              ...
              labs(title="Compare Means between Machines for Flatness difference with significant p-value", 
                   x="Flatness Features", y = "Mean value of Measurement")

# cylindricity feautures
Machine_cyl <-Machine_diff[,c(1,2,4,6)] %>% 
              ...
              position=position_dodge(), width=0.3) + theme_minimal() + geom_hline(yintercept = 0)+
              theme(axis.text = element_text(angle = 45,hjust=1,size = 8)) +scale_fill_manual(values=c('red3','salmon'))+
              labs(title="Compare Means between Machines for Cylindricity difference with significant p-value",
                  x="Cylindricity Features", y = "Mean value of Measurement")

# TP feautures
Machine_tp <- Machine_diff[,c(1,2,4,6)] %>% 
              ...
              position=position_dodge(), width=0.3) + theme_minimal() + geom_hline(yintercept = 0)+
              theme(axis.text = element_text(angle = 45,hjust=1,size = 8)) +scale_fill_manual(values=c('red3','salmon'))+
              labs(title="Compare Means between Machines for TP difference with significant p-value",
                   x="TP Features", y = "Mean value of Measurement")
```


```{r include= FALSE}
compare_machine_diff <- function(feature_category){
...
shinyUI <-pageWithSidebar(
  ...
  # Application title
  headerPanel("Compare Means across Machines if p value <0.05"),
  ...
  mainPanel(
                verbatimTextOutput("Compare significant differences across machines"),
                plotOutput("plot7")
          )
),
...
shinyServer <-(function(input, output) {
  ...
  output$plot7 <- renderPlot({
    compare_machine_diff(input$feature)
  })
...
shinyServer <-(function(input, output) {
  ...
  output$plot1 <- renderPlot({
    heatmap + coord_cartesian(xlim = ranges$x, ylim = ranges$y, expand = FALSE)
  })
...
shinyUI <-fluidPage(
    ...
    titlePanel("Part Summary of Bottom 40 performing parts having Maximum proportion of Failure"),
  ...
  fluidRow(
    column(4,
        selectInput("machine",
                    "Machine:",
                    c("All",
                      unique(as.character(part_aggregate_top20_failures$Machine_type))))
    ),
    column(4,
        selectInput("part",
                    "Part:",
                    c("All",
                      unique(as.character(part_aggregate_top20_failures$part_names))))
    ),
    column(4,
        selectInput("prop",
                    "Failure Proportion:",
                    c("All",
                      unique(as.character(part_aggregate_top20_failures$failure_prop))))
    )
  ),
...
shinyServer <-(function(input, output) {
 ...
 # Filter data based on selections
  output$table <- DT::renderDataTable(DT::datatable({
    data <- part_aggregate_top20_failures
    if (input$machine != "All") {
      data <- data[data$Machine_type == input$machine,]
    }
    if (input$part != "All") {
      data <- data[data$part_names == input$part,]
    }
    if (input$prop != "All") {
      data <- data[data$failure_prop == input$prop,]
    }
    data
  }))
...
shinyUI <-fluidPage(
    ...
    titlePanel("Part Summary of Top 40 performing parts having least proportion of Failure"),
  ...
  fluidRow(
    column(4,
        selectInput("machine",
                    "Machine:",
                    c("All",
                      unique(as.character(part_aggregate_bottom20_failures$Machine_type))))
    ),
    column(4,
        selectInput("part",
                    "Part:",
                    c("All",
                      unique(as.character(part_aggregate_bottom20_failures$part_names))))
    ),
    column(4,
        selectInput("prop",
                    "Failure Proportion:",
                    c("All",
                      unique(as.character(part_aggregate_bottom20_failures$failure_prop))))
    )
  ),
  # Create a new row for the table.
  fluidRow(
...
shinyServer <-(function(input, output) {
 ...
 # Filter data based on selections
  output$table <- DT::renderDataTable(DT::datatable({
    data <- part_aggregate_bottom20_failures
    if (input$machine != "All") {
      data <- data[data$Machine_type == input$machine,]
    }
    if (input$part != "All") {
...
tsne <- Rtsne(as.matrix(measure_unique), check_duplicates=FALSE, pca=FALSE, perplexity=50, theta=0.5, dims=3)
...
```{r include=FALSE}
data_3D <-tsne$Y %>% as.data.frame() %>% mutate(label = mydata2$cluster)
colnames(data_3D) <- c("X1","X2","X3","label")
```
...
# Define UI for dataset viewer application
shinyUI <-fluidPage(
  navbarPage("t-SNE Visualization of Part Analysis Data Clusters",
...
shinyServer <-(function(input, output) {
  ...
  output$plot <- renderPlotly({
    p <-plot_ly(type="scatter3d", x = Y[,1], y = Y[,2], z = Y[,3], color = as.factor(label),
...
```{r}
# library(shiny)
# shiny::runApp(system.file('/Users/swagatachakraborty/Documents/sc53711/Applications/Universities/UCSD/Assignments/my_Learning_folder/company_learning/Tesla/Tesla_Website', package='Swagata_App'))
```
